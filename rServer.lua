...
function M.isosurface(data, opts)
    opts = opts or {}
    ...
    shape = data:size()
    if nd == 3 then
        opts.size = data:size(3)
        data = data:reshape(data, shape[1], shape[2], shape[3])
    elseif nd == 4 then
        opts.size = data:size(3)
        data = data:reshape(data, shape[2], shape[3], shape[4])
    elseif nd == 5 then
        opts.size = data:size(3)
        data = data:reshape(data, shape[3], shape[4], shape[5])
    else
        print('Wrong Number of dimensions in data tensor. Please use 4 or 5 dimensions')
        return
    end

    opts.file = torch.totable(data)
    return pane('isosurface', opts.win, opts.title, opts)
